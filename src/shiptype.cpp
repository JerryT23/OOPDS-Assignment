...
//----------------------------------------------Cruiser----------------------------------------------------------------//
bool Cruiser::friendlyShip(Grid **grid, int shipAdditionX, int shipAdditionY, int shipPositionX, int shipPositionY)
{
    int locationX = shipPositionX + shipAdditionX;
    int locationY = shipPositionY + shipAdditionY;
    if (grid[locationY][locationX].getship() == nullptr)
...
void Cruiser::look(int x, int y, Grid **grid, int shipPositionX, int shipPositionY, int width, int height)
{
    position temp;
    for (int gridY = shipPositionY - 1; gridY <= shipPositionY + 1; gridY++)
    { // get the grid of nine-square area centered on (x,y)
...
void Cruiser::move(Grid **grid, int &shipPositionX, int &shipPositionY)
{
    if (availableMove.get_size() == 0)
    {
        std::cout << "Ship have nowhere to move!" << std::endl;
...
void Cruiser::ram(Grid **grid, int shipPositionX, int shipPositionY, int width, int height, queue &reenterQueue)
{
    for (int gridY = shipPositionY - 1; gridY <= shipPositionY + 1; gridY++)
    { // get the grid of nine-square area centered on (x,y)
        if (gridY < 0 || gridY >= height)
